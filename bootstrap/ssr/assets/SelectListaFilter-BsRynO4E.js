    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (isOpen.value && filteredOptions.value.length > 0) {
        ...
        ssrRenderList(filteredOptions.value, (option) => {
          _push(`<li class="p-2 hover:bg-gray-200 cursor-pointer">${ssrInterpolate(option.nome)}</li>`);
        });
        _push(`<!--]--></ul></div>`);
      } else {
        _push(`<!---->`);
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("resources/js/Pages/Campanha/Partials/SelectListaFilter.vue");
